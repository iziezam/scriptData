...
import {dbConnection,db } from "common";
...
async function CONNECTIONDB() {
  ...
  const port = Number(process.env.RUN_PORT);
  ...
  // start the server
  app.listen(port, ip, () => {
   console.log(`Server SQL SCRIPT is running at http://${ip}:${port}`);
  });
...
async function DataImport() {
  ...
  try {
    ...
    // return { code: CONST.SUCC, rs: result, rows: result.length };
  } catch (err) {
